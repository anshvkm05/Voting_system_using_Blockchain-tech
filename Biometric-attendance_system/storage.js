class FaceDataStorage {
  ...
  async saveFaceData(name, descriptors) {
    ...
    try {
      ...
      const fileHandle = await this.directoryHandle.getFileHandle(`${name}.json`, { create: true });
  ...
  async loadAllFaceData() {
    ...
    try {
      for await (const entry of this.directoryHandle.values()) {
        if (entry.kind === 'file' && entry.name.endsWith('.json') && !entry.name.startsWith('attendance_')) {
          try {
